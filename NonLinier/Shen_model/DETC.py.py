...
# ?----------------------------------------------------------------コスト関数
        self.Q = torch.eye(N, dtype=torch.double, device=device)
...
# ?----------------------------------------------------------------関数
    ...
    def f(self, x, u):
        Rx = torch.zeros(1, 1, dtype=torch.double).to(device)
        x1 = x[0]
        x2 = 0
...
xf = torch.tensor([0.0], dtype=torch.double, device=device)
# TODO グラフ保存
...
def calculate_cost(w, T, N, x0, Q, R, Qf, lam, K, h, Name, No):
    ...
    for i in range(T-1):
        u_t[:, i] = K@x_t[:, i]
        for j in range(N):
            x_t[j, i+1] = f(x_t[:, i], u_t[:, i])[j]+w[j, i]
        mark2[:, i] = 1
    for i in range(0, T-1):
