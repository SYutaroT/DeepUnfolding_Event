...
# ?----------------------------------------------------------------最適化パラメータ
        ...
        self.rho_e = nn.Parameter(torch.zeros(
            1, 1, dtype=torch.double, requires_grad=True, device=device))
...
# ?----------------------------------------------------------------関数
    ...
    def phi(self, x, Ix):  # !柔軟なトリガ
        return (x-Ix).T@(x-Ix)-self.rho_e**2*x.T@x
...
# ?----------------------------------------------------------------メイン
    ...
    def utrain(self):  # !最重要、ここで訓練
        ...
        cosine_scheduler = CosineAnnealingWarmRestarts(
            optimizer, T_0=10, T_mult=2, eta_min=1e-7)  # 学習率スケジューラ、今回は全ステップの1/5分学習率を減少、その後再度減少
...
def calculate_cost(w, T, N, x0, Q, R, Qf, lam, K, h, Name, No,rho_e):
    ...
    for i in range(0, T-1):
        if i == 0:
            ...
            if 0 < (x[:,i]-x_hat[:,i]).T@(x[:,i]-x_hat[:,i])-rho_e**2*x[:,i].T@x[:,i]:
                x_hat[:, [0]] = x[:, [i]]
                mark[:, i] = 1
                cnt += 1
            else:
...
for w in w_list:
    ...
    cost,  cnt, cost2 = calculate_cost(
        w, T, N, x0, Q, R, Qf, lam, K, h, imgname, counter,rho_e)
